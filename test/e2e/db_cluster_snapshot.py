...
"""Utilities for working with DB Cluster snapshot resources"""
...
ClusterSnapshotMatchFunc = typing.NewType(
    'ClusterSnapshotMatchFunc',
...
def status_matches(status: str) -> ClusterSnapshotMatchFunc:
    return StatusMatcher(status)


def wait_until(
        db_cluster_snapshot_id: str,
        match_fn: ClusterSnapshotMatchFunc,
    ...
    while not match_fn(get(db_cluster_snapshot_id)):
        if datetime.datetime.now() >= timeout:
            pytest.fail("failed to match DBClusterSnapshot before timeout")
...
def wait_until_deleted(
    ...
    Usage:
        from e2e.db_cluster_snapshot_id import wait_until_deleted

        wait_until_deleted(db_cluster_snapshot_id)
    ...
    while True:
        ...
        latest = get(db_cluster_snapshot_id)
        if latest is None:
            break
...
def get(db_cluster_snapshot_id):
    ...
    c = boto3.client('rds')
    try:
        resp = c.describe_db_cluster_snapshots(DBClusterSnapshotIdentifier=db_cluster_snapshot_id)
        assert len(resp['DBClusterSnapshots']) == 1
        return resp['DBClusterSnapshots'][0]
    except c.exceptions.DBClusterSnapshotNotFoundFault:
        return None


def get_tags(db_cluster_snapshot_arn):
    ...
    try:
        resp = c.list_tags_for_resource(
            ResourceName=db_cluster_snapshot_arn,
        )
        return resp['TagList']
    except c.exceptions.DBClusterSnapshotNotFoundFault:
        return None
