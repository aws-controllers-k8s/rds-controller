...
const (
	...
	SnapshotStatusUpgrading = "upgrading"
)
...
// snapshotAvailable returns true if the supplied resource is in the "available"
// state.
func snapshotAvailable(r *resource) bool {
	if r.ko.Status.Status == nil {
		return false
	}
	snapshotStatus := *r.ko.Status.Status
	return snapshotStatus == SnapshotStatusAvailable
}

// snapshotDeleting returns true if the supplied resource is in the "deleting"
// state.
func snapshotCreating(r *resource) bool {
	if r.ko.Status.Status == nil {
		return false
	}
	snapshotStatus := *r.ko.Status.Status
	return snapshotStatus == SnapshotStatusCreating
}
