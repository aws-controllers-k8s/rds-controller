...
func (r *resource) SetIdentifiers(identifier *ackv1alpha1.AWSIdentifiers) error {
	...
	r.ko.Spec.OptionGroupName = &identifier.NameOrID

	f0, f0ok := identifier.AdditionalKeys["engineName"]
	if f0ok {
		r.ko.Spec.EngineName = aws.String(f0)
	}
	f2, f2ok := identifier.AdditionalKeys["majorEngineVersion"]
	if f2ok {
		r.ko.Spec.MajorEngineVersion = aws.String(f2)
	}
...
func (r *resource) PopulateResourceFromAnnotation(fields map[string]string) error {
	primaryKey, ok := fields["optionGroupName"]
	if !ok {
		return ackerrors.NewTerminalError(fmt.Errorf("required field missing: optionGroupName"))
	}
	r.ko.Spec.OptionGroupName = &primaryKey

	f0, f0ok := fields["engineName"]
	if f0ok {
		r.ko.Spec.EngineName = aws.String(f0)
	}
	f2, f2ok := fields["majorEngineVersion"]
	if f2ok {
		r.ko.Spec.MajorEngineVersion = aws.String(f2)
	}
